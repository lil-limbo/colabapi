...
> **In one line:** `colabapi` gives you a persistent Colab terminal on your own server, using Google's official, ban-safe sign-in and tunnel — it never sees your Google password.
...
- ♻️ **Keep-alive that resists the idle timeout** — Google's official daemon does the primary keep-alive; `colabapi` adds a supervisory health check.
...
```
you ──> colabapi (this tool) ──> colab (Google's official CLI) ──> Google's tunnel ──> your Colab runtime
          │                                                                              (GPU/TPU VM)
          ├─ runtime picker, monitor, session timer, systemd service
          └─ never handles your Google password
```
...
| `colabapi daemon` | Supervisory keep-alive (used by the service). |
...
- The only things written to disk are **non-sensitive preferences and session bookkeeping** (which runtime you picked and when), under `~/.config/colabapi` and `~/.local/state/colabapi`.
...
- **Idle timeout (~90 min):** avoided by the keep-alive while a session is active.
...
Be a good citizen: don't hold GPU runtimes idle just to reserve them. Aggressive keep-alive can get an account flagged.
...
Allocate a runtime with `colabapi run`, then install the service (`colabapi service install`). Google's keep-alive holds off the idle timeout; the systemd service keeps `colabapi` supervising it after you log out.
